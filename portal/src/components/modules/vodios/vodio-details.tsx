...
const VodioDetails=({id}:{id:string})=>{
    const {useFetchBlogVodioDetails}=BlogsService();
    const {useHandleUpdateFeaturePermissions}=PermissionsService()
    const {data:blogDetails,isLoading:isBlogDetailsPending}=useFetchBlogVodioDetails(id);
    ...
    const {playList,setPlayList,metadata,setMetadata}=usePlayerStore();
    ...
    <div className={"p-[16px] w-full min-h-[832px] bg-[#0B0A0A]"}>
        <div className={"inline-flex items-center gap-[24px] flex-col md:flex-row"}>
            {/*<Image src={blogDetails?.img??"/dummy/dummy-02.png"} objectFit={"cover"} alt={"dummy"}*/}
            {/*       className={"w-4/5 md:w-[140px] md:h-[140px] object-cover rounded-[4px]"} width={140} height={140}/>*/}
            ...
            <ScrollArea
                ...
                {!isBlogDetailsPending && <div className={"pt-[16px]"}>
                    <Chrono
                        ...
                        cardHeight={100} lineWidth={1} borderLessCards={true} disableToolbar={true} mode={"VERTICAL"}
